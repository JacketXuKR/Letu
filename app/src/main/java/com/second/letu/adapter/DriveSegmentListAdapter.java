...
public class DriveSegmentListAdapter extends BaseAdapter {
    ...
    private List<DriveStep> mDriveStepList = new ArrayList<DriveStep>();

    public DriveSegmentListAdapter(Context context, List<DriveStep> list) {
        this.mContext = context;
        mDriveStepList.add(new DriveStep());
        for (DriveStep driveStep : list) {
            mDriveStepList.add(driveStep);
        }
        mDriveStepList.add(new DriveStep());
    }
    ...
    @Override
    public int getCount() {
        return mDriveStepList.size();
    }

    @Override
    public Object getItem(int position) {
        return mDriveStepList.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final DriveStep item = mDriveStepList.get(position);
        if (position == 0) {
            ...
            return convertView;
        } else if (position == mDriveStepList.size() - 1) {
