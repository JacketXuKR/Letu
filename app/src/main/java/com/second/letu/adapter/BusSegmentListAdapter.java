...
import com.second.letu.util.SchemeDriveStep;
...
public class BusSegmentListAdapter extends BaseAdapter {
    ...
    private List<SchemeDriveStep> mBusStepList = new ArrayList<SchemeDriveStep>();

    public BusSegmentListAdapter(Context context, List<BusStep> list) {
        this.mContext = context;
        SchemeDriveStep start = new SchemeDriveStep(null);
        start.setStart(true);
        mBusStepList.add(start);
        for (BusStep busStep : list) {
            if (busStep.getWalk() != null && busStep.getWalk().getDistance() > 0) {
                SchemeDriveStep walk = new SchemeDriveStep(busStep);//存储每一步是那种类型
                walk.setWalk(true);
                mBusStepList.add(walk);
            }
            if (busStep.getBusLine() != null) {
                SchemeDriveStep bus = new SchemeDriveStep(busStep);
                bus.setBus(true);
                mBusStepList.add(bus);
            }
            if (busStep.getRailway() != null) {
                SchemeDriveStep railway = new SchemeDriveStep(busStep);
                railway.setRailway(true);
                mBusStepList.add(railway);
            }

            if (busStep.getTaxi() != null) {
                SchemeDriveStep taxi = new SchemeDriveStep(busStep);
                taxi.setTaxi(true);
                mBusStepList.add(taxi);
            }
        }
        SchemeDriveStep end = new SchemeDriveStep(null);
        end.setEnd(true);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final SchemeDriveStep item = mBusStepList.get(position);
        if (position == 0) {
    ...
    private class ArrowClick implements View.OnClickListener {
        ...
        private SchemeDriveStep mItem;

        public ArrowClick(final ViewHolder holder, final SchemeDriveStep item) {
            mHolder = holder;
            mItem = item;
        }
